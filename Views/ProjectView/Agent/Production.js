...
import { updateImagesData } from "../../../Store/Label/ActionCreators";
...
class ProductionView extends React.Component {
                ...
                .then((response) => {
                    return new Promise((resolve, reject) => {
                        if (response === undefined){
                            reject({ message: "Project Not Available" })
                        }
                    ...
                    store.dispatch(setStatusBar(true, "error", "Server error on Getting Project Data, Try Again!"))
                    console.error(error);
                    this.props.history.push("/projects")
                })
        }
    ...
    handleStop = ()=>{
        ...
        .catch(error => {
            store.dispatch(setStatusBar(true, "error", "Server error in Stopping Task, Try Again later!"))
            console.error(error);
        })
    }
            ...
            <div>
                ...
                { this.state.workStarted &&
                    <Button variant="contained" className={classes.SubmitButton}
                        onClick={this.handleSubmit} color="secondary" disabled={this.state.submitting}
                    > Submit Answer
                </Button>
                }
            </div>
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(ProductionView));
