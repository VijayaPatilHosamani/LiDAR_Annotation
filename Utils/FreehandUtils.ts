...
import { LabelFreehand } from "../Store/Label/types";

export class FreehandUtils {
  ...
  public static drawExistingFreehand(
    canvas: HTMLCanvasElement,
    data: IEditor
  ): void {
    const imageData: ImageDataType | null = LabelsSelector.getActiveImageData();
    if (imageData) {
      imageData.labelFreeHand.forEach((labelPolygon: LabelFreehand) => {
        ...
        if (!labelPolygon.isHidden) {
          labelPolygon.Lines.forEach((element: any) => {
            ...
            const startOnCanvas: IPoint | null = RenderUtils.transferPointFromImageToCanvasContent(element.Line.start, data);
            const endOnCanvas: IPoint | null = RenderUtils.transferPointFromImageToCanvasContent(element.Line.end, data);
