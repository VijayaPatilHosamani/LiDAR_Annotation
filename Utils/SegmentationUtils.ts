...
import { LabelPolygon, LabelSegmentation } from "../Store/Label/types";
...
export class SegmentationUtils {
  ...
  public static drawExistingSegmentation(
    ...
    if (imageData) {
      let color = "red";
      imageData.labelSegmentation.forEach((labelPolygon: LabelSegmentation) => {
        color = labelPolygon.LineColor;
        if (!labelPolygon.isHidden) {


          var polygonPoints: Array<any> = [];
          labelPolygon.Lines.forEach(
            (element: any) => {
              //start
              ...
              if (startOnCanvas && endOnCanvas) {
                ...
                polygonPoints.push({
                  start: standardizedLine.start,
                  end: standardizedLine.end,
                });
              }
            } //end
          );
          DrawUtils.drawPolygon(
            canvas,
            polygonPoints,
            color,//  LineConfigs.LINE_INACTIVE_COLOR,
  ...
  public static drawExistingPolygon(
    canvas: HTMLCanvasElement,
    data: IEditor
  ): void {
    const imageData: ImageDataType | null = LabelsSelector.getActiveImageData();
